...
#if defined(_PLANAR_REFLECTION) || defined(_PLANAR_REFLECTION_WITH_REFLECTION_MATRIX)

TEXTURE2D(_PlanaReflectionTexture);
SAMPLER(sampler_PlanaReflectionTexture);
...
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, float clearCoatMask,
    ...
    half3 normalWS, half3 viewDirectionWS, float2 uv)
{
    ...
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfData.perceptualRoughness, occlusion);//half3(0,0,0);
    #if defined(_PLANAR_REFLECTION) || defined(_PLANAR_REFLECTION_WITH_REFLECTION_MATRIX)
        float2 screenUV = uv;
        #if defined(_PLANAR_REFLECTION_WITH_REFLECTION_MATRIX)
        float2 sampleUV = float2(screenUV.x, screenUV.y);
        #elif  defined(_PLANAR_REFLECTION)
        float2 sampleUV = float2(screenUV.x, 1 - screenUV.y);
        #endif
        indirectSpecular = SAMPLE_TEXTURE2D(_PlanaReflectionTexture, sampler_PlanaReflectionTexture, sampleUV).rgb * occlusion;
